...
single_user_object = {
        ...
        "last_recharge": "Rs. 1000 on 15-09-2019",
        "balance_usage": "Your total balance usage till now is Rs.630.",
        # "emergency_laon_last_availed": "You availed emergency loan on 10-09-2019",   # using slot for last_availed
        "roaming_status": "off",
...
def get_remaining_balance(mobile_number):
    if mobile_number not in user_data.keys():
        return "Sorry, No record found against this number: {}".format(mobile_number)

    return """Your current balance is: Rs. {balance}, validity: {balance_validity}""".format(
...
def get_balance_expiry(mobile_number):
    ...
    return """Your balance is valid till: {balance_validity}""".format(
        balance_validity=user_data[mobile_number]['balance_validity'])
...
def get_last_recharge_info(mobile_number):
    return """Last time you recharged {last_recharge}.\n
...
def get_last_recharge_date(mobile_number):
    return """Last time you recharged {last_recharge}.""".format(
        last_recharge=user_data[mobile_number]['last_recharge'])
