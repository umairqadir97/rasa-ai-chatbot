...
class HelloAndGreetings(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            authenticated_user_response_templates = ["Hi {user_name}, welcome to telenor customer care!",
                                                     "Hey {user_name}, how can i help you?"]
            user_name = ""
...
class ActionIntermediateFallback(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if last_intent_confidence < 0.3 or last_intent in [None, 'out_of_scope']:
            ...
            message = "Did you mean '{}'?".format(last_intent)
            buttons = [{'title': 'Yes',
        ...
        try:
            ...
            if previous_actions[-1] == "action_custom_fallback":
                if consecutive_fallback >= 2.0:
                    ...
                    response = "Sorry, I can't understand your question. Please wait, your query is being transferred to relevant expert..."
...
class OffersInformation(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            dispatcher.utter_message("Reply 'YES' to subscribe this offer.")
        except Exception as e:
            dispatcher.utter_message("Sorry, Something went wrong. Apna swal dobara enter krain")
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if mobile_number and pin:
            ...
            if mobile_number in user_data.keys() and len(pin) == 4:
                dispatcher.utter_message("\nHi {}, You are authenticated against mobile_number:{} successfully!".format(
                ...
                dispatcher.utter_message("Sorry, No record found against this number:{}".format(mobile_number))
            elif len(pin) != 4:
        ...
        dispatcher.utter_message("Authentication Failed. Repeat your query & enter correct credentials!")
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            if verification_status == "yes":
                ...
                response = "Your current balance is: Rs. {balance}, validity: {balance_validity}".format(
...
class BalanceExpiry(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            if verification_status == "yes":
                balance_validity = tracker.get_slot("current_balance_validity")
                response = "Your balance is valid till: {balance_validity}".format(balance_validity=balance_validity)
                dispatcher.utter_message(response)
        except Exception as e:
            dispatcher.utter_message("Sorry, Something went wrong. Apna swal dobara enter krain")
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            if verification_status == "yes":
                ...
                response = "Your total balance usage till now is Rs.{balance_usage}.".format(balance_usage=balance_usage)
                dispatcher.utter_message(response)
        except Exception as e:
            dispatcher.utter_message("Sorry, Something went wrong. Apna swal dobara enter krain")
            print("\nERROR: Something went wrong with BalanceExpiry: {}".format(e))
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            if verification_status == "yes":
                balance = tracker.get_slot("current_balance_value")
                balance_validity = tracker.get_slot("current_balance_validity")
                response = """Last time you recharged {last_recharge}.
                Your current balance is Rs. {balance}, validity {balance_validity}""".format(
...
class LastRechargeDate(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            verification_status = tracker.get_slot("verification")
            if verification_status == "yes":
                response = "Last time you recharged {last_recharge}.".format(
                    last_recharge=single_user_object['last_recharge'])
                dispatcher.utter_message(response)
        except Exception as e:
            dispatcher.utter_message("Sorry, Something went wrong. Apna swal dobara enter krain")
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            if verification_status == "yes":
                ...
                response = """Sorry, You haven't returned previous loan of Rs.20.
                You can not take more emergency balance till next recharge."""
                if loan_service_status == "unsubscribed":
                    response = "You haven't subscribed loan. Reply 'YES' to get emergency balance of Rs.20."
                dispatcher.utter_message(response)
        except Exception as e:
            dispatcher.utter_message("Sorry, Something went wrong. Apna swal dobara enter krain")
        ...
        try:
            ...
            if verification_status == "yes":
                ...
                if loan_service_status == "unsubscribed":
                    ...
                    response = "You have successfully availed emergency loan of Rs.20"
...
class EmergencyLoanLastAvailed(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        try:
            ...
            if verification_status == "yes":
                loan_last_availed = tracker.get_slot("loan_last_availed")
                response = "You availed emergency loan of Rs.20 on {}.".format(loan_last_availed)
                dispatcher.utter_message(response)
        except Exception as e:
            dispatcher.utter_message("Sorry, Something went wrong. Apna swal dobara enter krain")
